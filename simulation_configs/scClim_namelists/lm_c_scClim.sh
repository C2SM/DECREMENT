...
lm_c_INPUT_IO(){
 ...
 &IOCTL
  ...
  ytunit_restart = 'd'
  lbdclim = .TRUE.
  yncglob_title = "COSMO driven by ERA5-PGW-MPI-HR"
 ...
 &GRIBOUT
  yform_write = '${LM_NL_OUTPUT_FMT_C}'
  hcomb = 0.0, ${LM_NL_HSTOP}, 1
  yvarml='U_10M', 'V_10M', 'T_2M', 'RELHUM_2M', 'PS', 'PMSL', 'QV_2M',
         'CAPE_ML', 'CIN_ML','HZEROCL', 'HPBL',
         'CLCH', 'CLCM', 'CLCL', 'CLCT',
         'TQC', 'TQI','TQR','TQV','TQG','TQS',
         'ALHFL_S', 'ASHFL_S', 'ASOB_T',  'ASOD_T',  'ATHB_T','ALB_RAD',
         'ASOB_S', 'ASWDIFD_S','ASWDIR_S', 'ASWDIFU_S', 'ATHB_S', 'ATHD_S', 'ATHU_S',
  yvarpl = ' '
  yvarzl = ' '
  ireset_sums = 2
  lcheck = .TRUE.
...
&GRIBOUT
   yform_write = '${LM_NL_OUTPUT_FMT_C}',
   ncomb=$LM_NL_NSTART_C,$LM_NL_NSTOP_C,4,
   yvarml='TOT_PREC','TOT_SNOW',
   yvarpl=' ',
   yvarzl=' ',
   ireset_sums = 2,
   lcheck=.TRUE.,
 ...
 &GRIBOUT
  ...
  yvarml='VMAX_10M', 'W_SO','W_SNOW', 'H_SNOW', 'T_SO', 'TMIN_2M', 'TMAX_2M', 'RUNOFF_S', 'RUNOFF_G',
  yvarpl = ' '
  yvarzl = ' '
  ireset_winds = 2
  ireset_temps = 2
